...
contract Casino {
    ...
    // Sender should not be emmited => FIX
    //Amount should not be indexed we can calculate it on the frontend, we don't even need it => FIX
    event BetFinished(address indexed sender, uint256 indexed randomNumber, uint256 indexed amount);
    // event BetFinished(uint256 indexed randomNumber) => CORRECT
